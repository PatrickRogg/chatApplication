...
public class AdvancedMessageListenerThreadImpl extends AbstractMessageListenerThread {
	...
	protected void loginEventAction(ChatPDU receivedPdu) {
		...
		try {
			...
			// Sendet den Confirms der Clients an den Server
			ChatPDU loginConfirm = ChatPDU.createLoginConfirmActionPdu(sharedClientData.userName,receivedPdu);
			connection.send(loginConfirm);
			Thread.sleep(500);
	...
	protected void logoutEventAction(ChatPDU receivedPdu) {
		...
		try {
			handleUserListEvent(receivedPdu);
			ChatPDU logoutConfirm = ChatPDU.createLogoutConfirmActionPdu(sharedClientData.userName,receivedPdu);
	...
	protected void chatMessageEventAction(ChatPDU receivedPdu) {
		...
		try {
			ChatPDU chatMessageConfirm = ChatPDU.createChatMessageConfirmActionPdu(sharedClientData.userName,receivedPdu);
			
			connection.send(chatMessageConfirm);
			Thread.sleep(500);
		} catch (Exception e) {
			ExceptionHandler.logException(e);
		}

...
public class AdvancedChatWorkerThreadImpl extends AbstractWorkerThread {
	...
	protected void loginRequestAction(ChatPDU receivedPdu) {
		...
		if (!clients.existsClient(receivedPdu.getUserName())) {
			...
			// erzeugt eine Waitlist der eingeloggten Clients
			
			clients.createWaitList(receivedPdu.getUserName());
			System.out.println("Login: " + clients.getWaitList(receivedPdu.getUserName()));
	...
	protected void logoutRequestAction(ChatPDU receivedPdu) {
		...
		if (!clients.existsClient(userName)) {
			...
			clients.createWaitList(pdu.getUserName());
			System.out.println("Logout: " + clients.getWaitList(pdu.getUserName()));
	...
	protected void chatMessageRequestAction(ChatPDU receivedPdu) {
		...
		if (!clients.existsClient(receivedPdu.getUserName())) {
			...
			//erzeugt eine Waitlist für den Message-Confirm
			clients.createWaitList(receivedPdu.getUserName());
			System.out.println("Message: " + clients.getWaitList(receivedPdu.getUserName()) + "  UserName: " + receivedPdu.getUserName());
	...
	protected void handleIncomingMessage() throws Exception {
		...
		try {
			...
			case LOGIN_CONFIRM:
				// Login-Confirm vom Client empfangen
				clients.deleteWaitListEntry(receivedPdu.getEventUserName(), receivedPdu.getUserName());
				System.out.println("EventUser: " + receivedPdu.getEventUserName() + "  UserName: " + receivedPdu.getUserName() + "  Liste: " + clients.getWaitList(receivedPdu.getEventUserName()));
				if(clients.getWaitListSize(receivedPdu.getEventUserName()) == 0) {
					loginConfirmAction(receivedPdu);
					
					// Waitlist des Clients löschen
					clients.deleteWaitList(receivedPdu.getEventUserName());
				}
				break;
				
			case LOGOUT_CONFIRM:
				// Logout-Confirm vom Client empfangen
				clients.deleteWaitListEntry(receivedPdu.getEventUserName(), receivedPdu.getUserName());
				System.out.println("EventUser: " + receivedPdu.getEventUserName() + "  UserName: " + receivedPdu.getUserName() + "  Liste: " + clients.getWaitList(receivedPdu.getEventUserName()));
				if(clients.getWaitListSize(receivedPdu.getEventUserName()) == 0) {

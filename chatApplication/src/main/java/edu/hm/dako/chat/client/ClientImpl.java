...
public class ClientImpl extends AbstractChatClient {
	...
	public ClientImpl(ClientUserInterface userInterface, int serverPort,
		...
		if(SystemConstants.IMPL_TCP_SIMPLE.equals(serverType)) {
			try {
				// Simple TCP Client erzeugen
		...
		if(SystemConstants.IMPL_TCP_ADVANCED.equals(serverType)) {
			try {
				// Advanced TCP Client erzeugen
